...
export function MainTabs({
  ...
  return (
    <div>
      ...
      {activeTab === "action" && (
        <ActionList items={actionItems} onDetailOpen={setIsDetailOpen} />
      )}
      {activeTab === "books" && (
      ...
      <div className={`fixed bottom-20 right-5 z-40 flex flex-col items-center gap-2 transition-opacity duration-200 ${isDetailOpen ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
        ...
        {activeTab === "memo" && (
          <Link
            href="/notes/new"
            className="relative w-12 h-12 bg-[#d4a017] text-white rounded-full flex items-center justify-center shadow-lg hover:bg-[#b8890f] transition-colors"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
            </svg>
            <span className="absolute -top-0.5 -left-0.5 w-5 h-5 bg-white text-[#d4a017] rounded-full flex items-center justify-center text-s font-bold leading-none shadow">+</span>
          </Link>
        )}
        {activeTab === "books" && (
          <Link
              ...
              {items.map((item) => (
                ...
                >
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    {item.book && (
                      <span className="text-[10px] px-2 py-0.5 bg-[#f3f4f6] text-[#6b7280] rounded-full">{item.book.title}</span>
                    )}
                  </div>
